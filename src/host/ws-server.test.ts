 ...
 *
 * Ports: every bind here uses an ephemeral port (BROWSR_PORT=0 for the host,
 * port 0 for the server) — never the bridge default, which may be live.
 */
...
describe("ws-server frame helpers", () => {
  it("computes the RFC6455 accept key (sha1 + GUID)", () => {
...
describe("host.mjs stdio bridge", () => {
  it("forwards WS frames to framed stdout and framed stdin back to WS clients", async () => {
    ...
    const logDir = mkdtempSync(join(tmpdir(), "browsr-ws-test-"));
    ...
    // Logging went to the file with the browsr prefix; stderr stayed clean.
    const logText = readFileSync(logPath, "utf8");
    expect(logText).toMatch(/\[browsr\]/);
