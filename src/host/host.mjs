#!/usr/bin/env node
/**
 * browsr — native messaging host.
 *
 * Chrome spawns this via the NativeMessagingHosts manifest (written by
 * `browsr install --extension-id <id>`). It bridges the extension's native
 * messaging port (4-byte little-endian length-prefixed JSON on stdin/stdout)
 * to a localhost WebSocket server that the browsr MCP server connects to.
 *
 *   extension <-> [stdio, framed JSON] <-> host.mjs <-> [ws://127.0.0.1:8757] <-> MCP
 *
 * stdout is the protocol channel. Logs append to /tmp/browsr.log
 * (override with BROWSR_LOG) — stderr stays clean, because without
...
import { startBridgeServer, BROWSR_PORT } from "./ws-server.mjs";

const LOG_PATH = process.env.BROWSR_LOG || "/tmp/browsr.log";
 ...
 */
function log(msg) {
  const line = `[browsr] ${new Date().toISOString()} ${msg}\n`;
  try {
...
process.stdin.on("data", (chunk) => {
  ...
  while (stdinBuf.length >= 4) {
    ...
    try {
      ...
      if (msg?.type === "hello") writeStdout({ type: "hello", host: "browsr" });
