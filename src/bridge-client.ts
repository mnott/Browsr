/**
 * WebSocket client for the browsr bridge.
 *
 * Talks to the native host's hand-rolled WS server (ws://127.0.0.1:8757) with
...
export const BRIDGE_PORT = Number(process.env.BROWSR_PORT || 8757);
export const BRIDGE_URL = `ws://127.0.0.1:${BRIDGE_PORT}`;

export const NOT_CONNECTED_MESSAGE =
  "Browsr bridge not connected — start Chrome (with the Browsr extension loaded and its native host installed) and try again.";
...
export const globalWebSocketFactory: WebSocketFactory = (url) => {
  ...
  if (!WS) {
    throw new Error("this node runtime has no global WebSocket — browsr needs node >= 22");
  }
  return new WS(url);
};
